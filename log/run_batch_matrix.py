...
after an interruption continues where it left off.
    ...
    py -3 run_batch_matrix.py                       # P1 deck = AggroPirateWarrior
    py -3 run_batch_matrix.py --deck1 RenoKazakusMage
    py -3 run_batch_matrix.py --deck1 RenoKazakusMage --num-games 30

P1's deck can also be set via the BATCH_DECK1 env var (and BATCH_NUM_GAMES).
"""
import os
import csv
import time
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="Batch matrix runner for SabberStone simulations.")
    p.add_argument(
        "--deck1", default=os.environ.get("BATCH_DECK1", "AggroPirateWarrior"),
        help="P1 deck for every directory (default: AggroPirateWarrior). "
             "Any of the 9 decks in sabberstone_simulator.DECKS.")
    p.add_argument(
        "--num-games", type=int,
        default=int(os.environ.get("BATCH_NUM_GAMES", "20")),
        help="games per matchup (default: 20).")
    return p.parse_args()


def main():
    global DECK1, NUM_GAMES, GRAND_TOTAL
    args = _parse_args()
    if args.num_games < 1:
        raise SystemExit("--num-games must be >= 1")
    # _resolve_deck validates and returns the canonical name (sys.exit if unknown);
    # use it so the directory name + tags get the right casing.
    DECK1 = sim._resolve_deck(args.deck1)[0]
